...
既存の Fitlet 素材だけを使い、明るい配色・太い輪郭・フラットな面でまとめた日本語・英語のストア素材です。フィーチャーグラフィックはロゴと既存キャラクターだけでシンプルに、最初の2枚は1枚のホームマップ画面を大きく斜めに横たわせて左右2枚にまたがせています。iPad用画像はベゼルを使わず、アプリ画面そのものを大きく配置しています。編集元は `source/` にあります。iPhone用PNGは `node scripts/export-store-assets.mjs`、iPad用PNGは `node scripts/export-ipad-store-assets.mjs` で再出力できます。
...
- 元画像は `source/screens/ipad/ja/` / `source/screens/ipad/en/` です。iPad用の合成処理は `scripts/export-ipad-store-assets.mjs` にあります。
